...
### First Step: Calculating weights of indoor POIS ----------------------------------------------------------------------------
...
new_destinations <- baseline_destinations %>% 
  ...
  mutate(attribute = dest_type) %>% 
  rename("geom" = "GEOMETRY") %>%  
  select(-dest_type, -weight)

# get the positive POIs from original pois
poi_positive <- orgPoi %>%
...
poi_job <- poi %>%
  ...
  mutate(JobCategory = case_when(attribute %in% c("pipeline facility", "power facility") ~
                                 ...
                                 attribute %in% c( "child care", "primary school", "primary/secondary school", "secondary school", 
                                                   "kindergarten","primary") ~
                                   "Education.and.Training",
                                 attribute %in% c( "pharmacy", "maternal/child health centre", "hospital", 
                                                   "aged care","community health centre", "day procedure centre", 
