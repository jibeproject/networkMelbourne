...
keepCrossings <- function(nodes_current) {
  nodes <- nodes_current %>% # Changing to MATSim expected format
...
makeNetwork<-function(city, outputSubdirectory = "generated_network"){
  ...
  if (city == "Bendigo") {
    ...
    outputCrs = 28355
    osmGpkg = "./output/melbourne_osm.gpkg"
    unconfiguredSqlite = "data/network_victoria.sqlite"
    cropAreaPoly = "city-of-melbourne_victoria"
  ...
  system.time(networkFinalDirect <-
                makeEdgesDirect2(networkFinal[[1]],
                                 networkFinal[[2]],
                                 outputCrs))
